...
### أين تُخزَّن البيانات / Where the data lives

البيانات مخزّنة في **قاعدة البيانات (Neon / أي PostgreSQL)** — الجداول: `reports`,
`sections`, `statuses`, `projects` (انظر `db/schema.sql`).

- عند كل **نشر**، يقوم الخادم تلقائياً بإنشاء الجداول ومزامنة محتوى `reportsData.js`
  إلى قاعدة البيانات (upsert)، فيبقى محتوى Neon مطابقاً للكود.
- `/api/report` **يقرأ من قاعدة البيانات**، ويعود إلى ملف البيانات فقط إذا تعذّر الوصول
  للقاعدة (حتى لا تظهر صفحة فارغة أثناء بدء Neon من السكون).
- **للتسليم لتقنية المعلومات:** الملف `db/neon_seed.sql` يحتوي على البنية والبيانات كاملة
  وجاهز للتشغيل على أي PostgreSQL داخلي. وعند تولّي IT إدارة البيانات داخل القاعدة، اضبط
  متغيّر البيئة **`AUTO_SYNC=false`** لإيقاف الكتابة من الكود واعتماد القاعدة مصدراً وحيداً.
